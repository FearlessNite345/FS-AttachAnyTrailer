            ...
            if not IsVehicleAttachedToTrailer(Vehicle) then
                if GetEntityBoneIndexByName(Vehicle, 'attach_female') ~= -1 then
                    ...
                    if closestTrailer ~= nil and closestTrailer ~= 0 then
                        if config.autoAttachHitchToTrailer then
                            AttachVehicleToTrailer(Vehicle, closestTrailer, 1.2)
                        else
                            updateRate = 0
                            FS_Lib:DrawText2D(0.5, 0.8, '~g~[' .. FS_Lib:GetKeyString(config.attachKeybind) .. ']~w~ '  .. config.locales['attachToTrailer'], 0.6, true)
                            if IsControlJustPressed(0, config.attachKeybind) then
                                AttachVehicleToTrailer(Vehicle, closestTrailer, 1.2)
                            end
                        end
                    else
